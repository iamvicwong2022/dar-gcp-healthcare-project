...
CREATE TABLE IF NOT EXISTS `healthcare-dataanalytic-pro.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `healthcare-dataanalytic-pro.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `healthcare-dataanalytic-pro.silver_dataset.claims` c
